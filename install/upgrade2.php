    ...
    if (empty($_POST['username']) || empty($_POST['password'])) {
        ...
        if (!$error) {
            ...
            try {
                ...
                if (isset($config['edocument_format_no']) && preg_match('/^(.*?)%0([0-9]+)d$/', $config['edocument_format_no'], $match)) {
                    $config['edocument_prefix'] = $match[1];
                    $config['edocument_format_no'] = '%0'.$match[2].'d';
                }
                // ตาราง inventory
                ...
                if (!$db->fieldExists($table, 'job_id')) {
                    $db->query("ALTER TABLE `$table` ADD `job_id` VARCHAR(20) NULL DEFAULT NULL");
                    $db->query("ALTER TABLE `$table` ADD `comment` VARCHAR(1000) NULL DEFAULT NULL");
                    $content[] = '<li class="correct">ปรับปรุงตาราง `'.$table.'` สำเร็จ</li>';
                }
